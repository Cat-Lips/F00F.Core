...
public partial class MyInput
{
    ...
    public static void ShowWithMouse(Control ui) => AddShowMouseAction(() => ui.Visible = ShowMouse);
    public static void HideWithMouse(Control ui) => AddShowMouseAction(() => ui.Visible = !ShowMouse);
    public static void ShowWithMouse(CanvasLayer ui) => AddShowMouseAction(() => ui.Visible = ShowMouse);
    public static void HideWithMouse(CanvasLayer ui) => AddShowMouseAction(() => ui.Visible = !ShowMouse);
    ...
    public static bool MouseOrbit(Node3D self, InputEvent e, in Vector3 target, float sensitivity, float pitchLimit) => Active && self.MouseOrbit(e, target, sensitivity, pitchLimit);
    public static bool MouseOrbit(Node3D self, InputEventMouseMotion motion, in Vector3 target, float sensitivity, float pitchLimit) { if (!Active) return false; self.MouseOrbit(motion, target, sensitivity, pitchLimit); return true; }
