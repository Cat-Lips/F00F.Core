...
[Tool]
public partial class Cam3D : Camera3D
{
    #region Private
    ...
    [Export] public Cam3DInput Input { get; set => this.Set(ref field, value ?? new()); }
    [Export] public Cam3DConfig Config { get; set => this.Set(ref field, value ?? new()); }
    ...
    public sealed override void _Ready()
    {
        ...
        void InitInput()
            => MyInput.OnMouseVisibilityChanged(() => SelectMode = MyInput.GetMouseVisible());
        ...
        void OnSelectModeSet()
        {
            ...
            {
                if (myTarget is IActive a) a.Active = false;
                MyInput.SetMouseVisible(true);
            }
            else
            {
                if (myTarget is IActive a) a.Active = true;
                GetViewport().GuiReleaseFocus();
                MyInput.SetMouseVisible(false);
            }
        }
