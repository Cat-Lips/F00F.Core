...
namespace F00F
{
    ...
    public partial class Camera : Camera3D
    {
        ...
        public override void _Process(double delta)
        {
            ...
            bool FollowTarget()
            {
                if (myTarget is null) return false;
        ...
        {
            if (HandleSelectModeToggle()) return;

            if (SelectMode)
            {
                if (HandleRayCastRequest()) return;
            }
            else
            {
            ...
            bool HandleSelectModeToggle()
                => this.Handle(Input.ToggleSelectMode(), () => SelectMode = !SelectMode);

            bool HandleRayCastRequest()
                => this.Handle(Input.SelectJustPressed(), () => RayCastRequested = true);

            bool HandleReset()
            {
                return this.Handle(Input.ResetCameraPosition(), ResetCameraPosition, ResetZoom);
            ...
            {
                if (myTarget is null)
                    return this.Handle(this.MouseRotate(e, Config.Sensitivity, Config.PitchLimit));
                else if (Input.RotateAroundTarget())
                    return this.Handle(this.MouseRotateAround(e, MyFollowTarget.GlobalPosition, Config.Sensitivity, Config.PitchLimit));
                return false;
            }
        }
