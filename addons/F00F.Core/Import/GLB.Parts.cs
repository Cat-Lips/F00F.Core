...
public static partial class GLB
{
        ...
        Action<Node3D> OnPartAdded = null)
    {
        ...
        if (mode is MeshMode.Static) rotation = GlbRotate.None; // Rotate model externally and (for now) refresh manually
        ...
        void AddPart(MeshInstance3D part)
        {
            ...
            var shapeModifier = xform.Scaled(Vector3.One * (1 - shapeReductionRatio)); // Broken?
            var shapes = CreateShapes(part.Mesh, shapeModifier, shapeType, shapeCount);
