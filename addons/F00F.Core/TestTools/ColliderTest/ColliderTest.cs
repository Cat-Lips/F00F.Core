...
public partial class ColliderTest : Node
{
    ...
    public static class Enums
    {
        ...
        private static BodyType[] BodyTypes { get; } = Enum.GetValues<BodyType>().Except([BodyType.Random]).ToArray();
    ...
    private void Launch()
    {
        ...
        {
            body.GetNode<TestController>("Controller").Init(Shape, Size, Life, Mass);
            ...
            float Force()
            {
                ...
                if (MyInput.IsKeyLabelPressed(Key.Alt)) force *= this.Force;
                if (MyInput.IsKeyLabelPressed(Key.Ctrl)) force *= this.Force;
                if (MyInput.IsKeyLabelPressed(Key.Shift)) force *= this.Force;
