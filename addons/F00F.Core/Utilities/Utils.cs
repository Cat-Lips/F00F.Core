using System;
using System.Collections.Generic;
using System.Diagnostics;
using System.IO;
using System.Linq;
using System.Reflection;
using System.Runtime.CompilerServices;
using Godot;

namespace F00F;

public static partial class Utils
{
    #region New Scene

    public static Node New(this PackedScene scene) => scene.New<Node>();
    public static T New<T>(this PackedScene scene) where T : Node => (T)scene.Instantiate();

    public static Node New(this PackedScene scene, Action<Node> init) => scene.New<Node>(init);
    public static T New<T>(this PackedScene scene, Action<T> init) where T : Node { var x = scene.New<T>(); init(x); return x; }

    public static T New<T>() where T : Node => LoadScene<T>().New<T>();
    public static T New<T>(Action<T> init) where T : Node => LoadScene<T>().New(init);

    public static T New<T>(this T _) where T : Node => New<T>();
    public static T New<T>(this T _, Action<T> init) where T : Node => New(init);

    #endregion

    #region Load Res

    public static PackedScene LoadScene<T>(this T _) where T : Node => LoadScene<T>();
    public static PackedScene LoadScene<T>() where T : Node
        => Load<PackedScene>(GetScriptPath<T>(".tscn"));

    public static Shader LoadShader<T>(this T _, string path) where T : GodotObject => LoadShader<T>(path);
    public static Shader LoadShader<T>(string path) where T : GodotObject
        => Load<Shader>(GetScriptDir<T>().PathJoin(path.AddSuffix(".gdshader")));

    public static Shader LoadShader<T>(this T _) where T : GodotObject => LoadShader<T>();
    public static Shader LoadShader<T>() where T : GodotObject
        => Load<Shader>(GetScriptPath<T>(".gdshader"));

    public static Texture2D LoadTexture<T>(this T _, string path) where T : GodotObject => LoadTexture<T>(path);
    public static Texture2D LoadTexture<T>(string path) where T : GodotObject
        => Load<Texture2D>(GetScriptDir<T>().PathJoin(path));

    public static ConfigFile LoadConfig<T>(this T _, bool autoSave = true) where T : GodotObject => LoadConfig<T>(autoSave);
    public static ConfigFile LoadConfig<T>(bool autoSave = true) where T : GodotObject
        => GetScriptPath<T>(".cfg").LoadConfig(autoSave);

    public static T LoadRes<T>(this T _) where T : Resource => LoadRes<T>();
    public static T LoadRes<T>() where T : Resource
        => Load<T>(GetScriptPath<T>(".tres"));

    public static string GetScriptPath<T>(string ext) where T : GodotObject
        => Path.ChangeExtension(GetScriptPath<T>(), ext);

    public static string GetScriptPath<T>() where T : GodotObject
        => typeof(T).GetCustomAttribute<ScriptPathAttribute>(false).Path;

    public static string GetScriptDir<T>() where T : GodotObject
        => GetScriptPath<T>().GetBaseDir();

    public static T Load<T>(string path) where T : Resource
    {
        Debug.Assert(ResourceLoader.Exists(path), $"Missing Resource: {path}!");
        return GD.Load<T>(path);
    }

    #region TryLoad

    public static T TryLoad<T>(string path) where T : Resource
        => ResourceLoader.Exists(path) ? GD.Load<T>(path) : null;

    public static Shader TryLoadShader(this GodotObject src) => src.TryLoad<Shader>(".gdshader");
    private static T TryLoad<T>(this GodotObject src, string ext) where T : Resource
    {
        var srcType = src.GetType();
        var srcPath = srcType.GetCustomAttributes<ScriptPathAttribute>().Single(x => x.Path.EndsWith($"/{srcType.Name}.cs")).Path;
        var resPath = Path.ChangeExtension(srcPath, ext);
        return TryLoad<T>(resPath);
    }

    public static Texture2D TryLoadTexture<T>(this T _, string path) where T : GodotObject => TryLoadTexture<T>(path);
    public static Texture2D TryLoadTexture<T>(string path) where T : GodotObject
        => TryLoad<Texture2D>(GetScriptDir<T>().PathJoin(path));

    #endregion

    #endregion

    #region Preload

    public static T New<T>(this ResourcePreloader preload) where T : Node
        => preload.Get<T>().New<T>();

    public static PackedScene Get<T>(this ResourcePreloader preload) where T : Node
        => preload.Get<PackedScene>(typeof(T).Name);

    public static T New<T>(this ResourcePreloader preload, StringName name) where T : Node
        => preload.Get<PackedScene>(name).New<T>();

    public static T Get<T>(this ResourcePreloader preload, StringName name) where T : Resource
        => (T)preload.GetResource(name);

    public static IEnumerable<PackedScene> Get(this ResourcePreloader preload)
        => preload.GetResourceList().Select(x => preload.Get<PackedScene>(x));

    #endregion

    #region Get Res

    public static string GetScriptPath([CallerFilePath] string CallerFilePath = null)
        => ProjectSettings.LocalizePath(CallerFilePath);

    public static string GetScriptDir([CallerFilePath] string CallerFilePath = null)
        => GetScriptPath(CallerFilePath).GetBaseDir();

    public static string GetResPath(string RelativePath, [CallerFilePath] string CallerFilePath = null)
        => GetScriptDir(CallerFilePath).PathJoin(RelativePath);

    public static T GetRes<T>(string RelativePath, [CallerFilePath] string CallerFilePath = null) where T : Resource
        => Load<T>(GetResPath(RelativePath, CallerFilePath));

    #endregion
}
