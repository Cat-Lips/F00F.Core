...
public static partial class Utils
{
    ...
    public static string GetScriptPath<T>(string ext) where T : GodotObject
        => GetScriptPath<T>().Replace(".cs", ext);
    ...
    public static T Load<T>(this ResourcePreloader preloads, StringName name) where T : Resource
        => (T)preloads.GetResource(name);

    public static PackedScene Get<T>(this ResourcePreloader preloads)
        => preloads.Load<PackedScene>(typeof(T).Name);

    public static T New<T>(this ResourcePreloader preloads) where T : Node
        => (T)preloads.Get<T>().Instantiate();
}
