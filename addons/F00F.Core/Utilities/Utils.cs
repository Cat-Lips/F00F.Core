...
{
    public static T New<T>(this T _) where T : GodotObject => New<T>();
    public static T New<T>() where T : GodotObject => LoadScene<T>().Instantiate<T>();
    public static T New<T>(Action<T> init) where T : GodotObject { var x = New<T>(); init(x); return x; }
    ...
    public static ConfigFile LoadConfig<T>(bool autoSave = true) where T : GodotObject
        => GetScriptPath<T>().Replace(".cs", ".cfg").LoadConfig(autoSave);
