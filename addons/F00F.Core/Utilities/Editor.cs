...
public static partial class Editor
{
    ...
    //    {
    //        if (Editor.DisplayOnly(property, PropertyName.Config)) return;
    //    }
    //}
    ...
    //    {
    //        Editor.OnSave(what, OnSave));
    //
    //        void OnSave()
    //        {
    //            if (this.IsEditedSceneRoot())
    //                Editor.DoPreSaveReset(this, PropertyName.X);
    //            Editor.DoPreSaveReset(this, PropertyName.Z);
    //            return;
    //        }
    //    }
    //}
    ...
    public static void OnSave(int what, Action OnSave)
    {
        if (OnPreSave(what))
            OnSave();
        else if (OnPostSave(what))
            DoPostSaveRestore();
    }
    ...
    public static void DoPreSaveReset(GodotObject source, StringName property, Variant dflt = default)
    {
        var value = source.Get(property);
        source.Set(property, dflt);
    ...
    public static void DoPreSaveReset(GodotObject[] source, StringName property, Variant dflt = default)
    {
        foreach (var item in source)
            DoPreSaveReset(item, property, dflt);
    }

    public static void DoPreSaveResetShader(ShaderMaterial source, StringName property, Variant dflt = default)
    {
        var value = source.GetShaderParameter(property);
        source.SetShaderParameter(property, dflt);
        AddPostSaveAction(() => source.SetShaderParameter(property, value));
    }
    ...
    public static void DoPreSaveResetField<T, V>(T source, StringName property, in V @dflt) where T : GodotObject
    {
        var field = GetBackingField(source, property);

        var value = field.GetValue(source);
        field.SetValue(source, @dflt);
        AddPostSaveAction(() => field.SetValue(source, value));
    }
    ...
    public static void DoPreSaveReset(Action action)
        => AddPostSaveAction(action);
