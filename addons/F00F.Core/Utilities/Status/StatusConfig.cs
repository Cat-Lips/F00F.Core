...
public partial class StatusConfig : CustomResource
{
    ...
    private StringName FontColor = "font_color";
    public TextureRect Icon(Status status) => UI.TextureRect("Icon", Texture(status));
    public LineEdit Text(Status status, string msg) => UI.SunkLabel("Text", msg, Color(status));
    public Button Btn(Status status, string msg, Action action) => UI.Button("Text", action, msg, flat: true).SetThemeColor(FontColor, Color(status));
