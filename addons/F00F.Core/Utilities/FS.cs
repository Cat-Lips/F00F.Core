...
public static class FS
{
    ...
    public static string[] ListRes(string path) => ResourceLoader.ListDirectory(path).Select(path.PathJoin).ToArray();
    ...
    public static void CreateFile(string path)
    {
        using (FileAccess.Open(path, FileAccess.ModeFlags.Write))
            FileAccess.GetOpenError().Throw("CreateFile", path);
    }

    public static void RemoveFile(string path, bool force = false)
    {
        if (force) FileAccess.SetReadOnlyAttribute(path, false).Throw("RemoveFile", path);
        DirAccess.RemoveAbsolute(path).Throw("RemoveFile", path);
    }

    public static void MakeDir(string path)
        => DirAccess.MakeDirRecursiveAbsolute(path).Throw("MakeDir", path);
