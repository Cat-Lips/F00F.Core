...
public static class New
{
    ...
    public static ArrayMesh TerrainMesh(in Vector2I size, float amplitude, Func<float, float, float> RawHeight, Func<float, Color> RawColor = null, Func<float, float> RawGradient = null)
    {
        ...
        Array GetSurfaceData(in Vector2I size)
        {
            ...
            if (UseColor) arrays[(int)ArrayType.Color] = colors;
    ...
    public static ArrayMesh TerrainMesh(int size, float amplitude, Func<float, float, float> RawHeight, Func<float, Color> RawColor = null, Func<float, float> RawGradient = null)
        => TerrainMesh(Vector2I.One * size, amplitude, RawHeight, RawColor, RawGradient);

    public delegate void GetHeightData(int x, int z, out float height, out Color color);
    public static ArrayMesh HeightMesh(in CT ct, int size, GetHeightData GetHeightData)
    {
        ...
        var iGridSize = size;
        var iHalfSize = size / 2;
        var fGridSize = (float)iGridSize;

        AddVertices(ct);
        IndexVertices(ct);
        return GetMesh(ct);

        void AddVertices(in CT ct)
        {
            if (ct.Cancelled()) return;

            for (var z = -iHalfSize; z <= iHalfSize; ++z)
            {
                for (var x = -iHalfSize; x <= iHalfSize; ++x)
                {
                    GetHeightData(x, z, out var height, out var color);

                    st.SetColor(color);
                    st.SetUV(new((x + iHalfSize) / fGridSize, (z + iHalfSize) / fGridSize));
                    st.AddVertex(new(x, height, z));
                }
            }
        }

        void IndexVertices(in CT ct)
        {
            if (ct.Cancelled()) return;

            var rowVerts = iGridSize + 1;
            var nextRowVert = rowVerts + 1;
            for (var z = 0; z < iGridSize; ++z)
            {
                for (var x = 0; x < iGridSize; ++x)
                {
                    var i = z * rowVerts + x;

                    st.AddIndex(i);
                    st.AddIndex(i + 1);
                    st.AddIndex(i + rowVerts);

                    st.AddIndex(i + 1);
                    st.AddIndex(i + nextRowVert);
                    st.AddIndex(i + rowVerts);
                }
            }
        }

        ArrayMesh GetMesh(in CT ct)
        {
            if (ct.Cancelled()) return null;
    ...
    public static void HeightMesh(in CT ct, int size, GetHeightData GetHeightData, out Mesh mesh, out Shape3D shape)
    {
        mesh = New.HeightMesh(ct, size, GetHeightData);
        //shape = ct.Cancelled() ? null : new ConcavePolygonShape3D() { Data = mesh.GetFaces() };
        shape = null;
    }
    ...
    {
        Points = (Vector3[])new PlaneMesh { Size = size }.GetMeshArrays()[(int)ArrayType.Vertex],
    };
    ...
    private static Vector3[] Vertices(this Mesh mesh, int surface = 0) => [.. ((Vector3[])mesh.SurfaceGetArrays(surface)[(int)ArrayType.Vertex]).Distinct()];

    #endregion

    #region Shaders

    public static ShaderMaterial ShaderMaterial<T>() where T : GodotObject => ShaderMaterial(Utils.LoadShader<T>());
    public static ShaderMaterial ShaderAsMaterial<T>() where T : GodotObject => ShaderMaterial(Utils.LoadShader<T>());

    public static ShaderMaterial ShaderMaterial<T>(string path) where T : GodotObject => ShaderMaterial(Utils.LoadShader<T>(path));
    public static ShaderMaterial ShaderAsMaterial<T>(string path) where T : GodotObject => ShaderMaterial(Utils.LoadShader<T>(path));

    public static ShaderMaterial ShaderMaterial(Shader shader = null)
        => new() { Shader = shader ?? new() };

    public static Texture2DArray Texture2DArray(Array<Image> images, bool? validate = null)
    {
        var textureArray = new Texture2DArray();
        if (validate ?? Editor.IsEditor) Validate();
        var err = textureArray.CreateFromImages(images);
        if (err.NotOk()) ReportError($"{err}");
        return textureArray;

        void Validate()
        {
            if (images.IsNull()) { ReportError("Null image array"); return; }

            if (images.IsEmpty()) { ReportError("Empty image array"); return; }

            var first = images.FirstOrDefault(x => x.NotNull());
            if (first.IsNull()) { ReportError("No valid images"); return; }

            var width = first.GetWidth();
            var height = first.GetHeight();
            var format = first.GetFormat();
            var mips = first.HasMipmaps();

            images.ForEach((img, idx) =>
            {
                if (img.IsNull()) { ReportError($"Null Image [Idx: {idx}]"); return; }

                if (img.GetWidth() != width) ReportError($"Width Mismatch [Expected {width}, Actual: {img.GetWidth()}, File: {img.ResourcePath.GetFile()}, Idx: {idx}]");
                if (img.GetHeight() != height) ReportError($"Height Mismatch [Expected {height}, Actual: {img.GetHeight()}, File: {img.ResourcePath.GetFile()}, Idx: {idx}]");
                if (img.GetFormat() != format) ReportError($"Format Mismatch [Expected {format}, Actual: {img.GetFormat()}, File: {img.ResourcePath.GetFile()}, Idx: {idx}]");
                if (img.HasMipmaps() != mips) ReportError($"Mipmap Mismatch [Expected {mips}, Actual: {img.HasMipmaps()}, File: {img.ResourcePath.GetFile()}, Idx: {idx}]");
            });
        }

        void ReportError(string msg)
            => GD.PushWarning($"[New.Texture2DArray] {msg}");
    }

    public static Texture2DArray Texture2DArray(params IEnumerable<Image> images)
        => New.Texture2DArray(new Array<Image>(images));

    public static Texture2DArray Texture2DArray(params IEnumerable<Texture2D> textures)
        => New.Texture2DArray(textures.Select(x => x.GetImageOrNull()));
    ...
    private static ArrayMesh ArrayMesh(Vector3[] vertices, Vector3[] normals, int[] indices, Vector2[] uvs)
    {
        ...
        var mesh = new ArrayMesh();
        mesh.AddSurfaceFromArrays(PrimitiveType.Triangles, surfaceArray);
        return mesh;
    }
