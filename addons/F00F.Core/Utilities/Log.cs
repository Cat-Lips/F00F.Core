...
{
    public enum LL { Debug, Info, Warn, Error, None }
    public static LL LogLevel { get; set; } = LL.Debug;
    ...
    [Conditional("DEBUG")]
    public static void Debug(object msg = null, [CallerFilePath] string filePath = null, [CallerMemberName] string memberName = null) => Print(LL.Debug, filePath, memberName, msg);
    public static void Info(object msg = null, [CallerFilePath] string filePath = null, [CallerMemberName] string memberName = null) => Print(LL.Info, filePath, memberName, msg);
    public static void Warn(object msg = null, [CallerFilePath] string filePath = null, [CallerMemberName] string memberName = null) => Print(LL.Warn, filePath, memberName, msg);
    public static void Error(object msg = null, [CallerFilePath] string filePath = null, [CallerMemberName] string memberName = null) => Print(LL.Error, filePath, memberName, msg);
    public static void PushWarn(object msg = null, [CallerFilePath] string filePath = null, [CallerMemberName] string memberName = null) => Push(LL.Warn, filePath, memberName, msg);
    public static void PushError(object msg = null, [CallerFilePath] string filePath = null, [CallerMemberName] string memberName = null) => Push(LL.Error, filePath, memberName, msg);
    public static void Custom(string ll, object msg = null, [CallerFilePath] string filePath = null, [CallerMemberName] string memberName = null) => Print(ll, filePath, memberName, msg);

    //[Conditional("DEBUG")]
    public static IDisposable Timer(string data = null, int ms = -1, [CallerFilePath] string filePath = null, [CallerMemberName] string memberName = null)
    {
#if DEBUG
        ...
        return new DisposableAction(() =>
        {
            ...
            {
                var x = EnableMemberName;
                EnableMemberName = true;
                Print(LL.Debug, filePath, memberName, $"[PROFILE] {sw.Elapsed.HumaniseTime()}{data.Str()}");
                EnableMemberName = x;
            }
        });
#else
    ...
    private static void Print(string ll, string filePath, string memberName, object msg)
        => GD.PrintS($"[{ll}]", Format(filePath, memberName, msg));

    private static void Print(LL ll, string filePath, string memberName, object msg)
    {
        if (ll >= LogLevel)
            Print($"{ll}".ToUpper(), filePath, memberName, msg);
    }

    private static void Push(LL ll, string filePath, string memberName, object msg)
    {
        if (ll >= LogLevel)
        {
            switch (ll)
            {
                case LL.Warn: GD.PushWarning(Format(filePath, memberName, msg)); break;
                case LL.Error: GD.PushError(Format(filePath, memberName, msg)); break;
            }
        }
    }
    ...
    private static string ThreadId() => EnableThreadId ? $"[THRD{System.Environment.CurrentManagedThreadId}] " : null;
    private static string FileName(string x) => EnableFileName ? $"[{x.GetFileName()}] " : null;
        ...
        [CallerArgumentExpression(nameof(value9))] string key9 = null)
    {
        ...
        void PrintParts()
        {
            ...
            {
                var pad = Keys().DefaultIfEmpty().Max(x => x.Length);
