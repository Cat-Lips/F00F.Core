...
public static partial class UI
{
    public interface IBuilder
    {
        ...
        void AddGroup(string title, string prefix = null, bool fold = false, string check = null, string hint = null);
        void EndGroup();
    ...
    private class BuilderUI : GroupBuilder, IBuilder
    {
        ...
        public void AddGroup(string title, string prefix, bool fold, string check, string hint)
        {
            ...
            if (check.NotNull())
            {
                hint ??= check.TryTrimPrefix(prefix).Capitalise();
                var edit = GetEditControl(check, hint);
                root.AddChild(edit, forceReadableName: true);
                edit.Toggled += on =>
                {
                    group.ButtonPressed = on;
                    group.Disabled = !on;
                };

                CheckButton GetEditControl(string name, string hint)
                {
        ...
        public void AddOption(string name, string hint, params string[] items)
            => AddOption(name, hint, items.Select((x, i) => (x, i)).ToArray());

        public void AddOption(string name, string hint, params (string Name, int Id)[] items)
        {
