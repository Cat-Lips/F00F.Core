...
using static FileDialog;
using FileSelect = (Container Root, LineEdit FilePreview, Button FileClear, Button FileSelect, FileDialog FileDialog);
using Indent = (Container Root, Control Indent, Control Content);
using Layout = (Container Root, Control[] Content);
using MultiEdit = (Container Root, SpinBox[] EditControls);
using Range = (float? Min, float? Max, float? Step);
using ToggleEdit = (Container Root, Button Toggle);
using ToggleTitle = (Container Root, Button Toggle, Label Label);
...
public static partial class UI
{
    ...
    {
        var button = NewButton(name, $"{text}").SetSquareWidth();
    ...
    private static partial class Utils
    {
        ...
        public partial class TexturePreview(string name) : PreviewPanel(name)
        {
            protected sealed override Control GetPreview(string tooltip)
            {
                ...
                var label = SunkLabel("Text", tooltip);
                var texture = TextureRect("Texture", tooltip);
        ...
        public partial class CurvePreview(string name) : PreviewPanel(name)
        {
            ...
            protected sealed override Control GetPreview(string tooltip)
                => preview = curve is null ? null : TextureRect("Texture", GetTexture());
