...
namespace F00F
{
    public static partial class UI
    {
        ...
        public static Label NewLabel(string name, string hint = null) => NewLabel(name, null, hint);
        public static Label NewLabel(string name, string text, string hint) => new()
        {
            Name = $"{name}Label",
        ...
        public static OptionButton NewOptionEdit<T>(string name = null, string hint = null, Action<T> OnSelect = null) where T : struct, Enum
        {
            var ec = NewOptionEdit(name ?? typeof(T).Name, hint, Items<T>());
            if (OnSelect is not null) ec.ItemSelected += x => OnSelect((T)(object)x);
            return ec;
        }
