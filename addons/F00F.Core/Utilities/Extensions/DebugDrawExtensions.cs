...
public static class DebugDraw
{
    ...
    [Conditional("DEBUG_DRAW")]
    public static void DebugNormal(this Node3D self, in Vector3 normal, Godot.Color? color = null, Scope scope = Scope.Frame)
    {
        if (!Enabled) return;
        if (normal.Rounded(CutoffPrecision) == Vector3.Zero) return;
        Draw(self.GlobalPosition, normal, color ?? Color.Normal, scope);
    }
    ...
    private static class Color
    {
        ...
        public static readonly Godot.Color Normal = Colors.Yellow.With(a: .5f);
