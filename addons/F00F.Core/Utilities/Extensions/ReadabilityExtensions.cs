...
{
    public static void Enable(this SpinBox source, bool enable = true)
        => source.Editable = enable;

    public static void Enable(this BaseButton source, bool enable = true)
        => source.Disabled = !enable;

    public static void Enable(this CollisionShape3D source, bool enable = true)
        => source.Disabled = !enable;

    public static void Enable(this Node3D source, bool enable = true)
    {
        source.Visible = enable;
        source.RecurseChildren<CollisionShape3D>()
            .ForEach(shape => shape.Enable(enable));
    }
