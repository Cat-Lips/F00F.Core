...
public static class ReadabilityExtensions
{
    ...
    public static bool IsReady(this Node source) => source.IsNodeReady();
    public static bool NotReady(this Node source) => !source.IsNodeReady();
    public static bool IsInTree(this Node source) => source.IsInsideTree();
    public static bool NotInTree(this Node source) => !source.IsInsideTree();
    ...
    public static ShaderMaterial GetShader(this PlaneMesh source)
        => source.Material as ShaderMaterial;

    public static PlaneMesh GetPlaneMesh(this Node source, NodePath path)
        => (PlaneMesh)source.GetNode<MeshInstance3D>(path).Mesh;

    public static ShaderMaterial GetShader(this MeshInstance3D source)
        => (ShaderMaterial)source.GetActiveMaterial(0);

    public static ShaderMaterial GetShader(this Node source, NodePath path)
        => source.GetNode<MeshInstance3D>(path).GetShader();

    public static ShaderMaterial GetShader2D(this Node source, NodePath path)
        => (ShaderMaterial)source.GetNode<CanvasItem>(path).Material;
}
