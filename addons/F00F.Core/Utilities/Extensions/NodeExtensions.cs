...
public static class NodeExtensions
{
    ...
    public static void DetachChild(this Node source, bool free = true)
        => source.GetParent().RemoveChild(source, free: free);

    public static void Detach(this Node source, bool free = true)
        => source.DetachChild(free);
    ...
    public static void RemoveChildren(this Node source, Func<Node, bool> where)
        => source.GetChildren().Where(where).ForEach(x => source.RemoveChild(x, free: true));

    public static void RemoveChildren<T>(this Node source, Func<T, bool> where)
        => source.GetChildren<T>().Where(where).ForEach(x => source.RemoveChild(x as Node, free: true));
