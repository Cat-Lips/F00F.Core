...
public static class NodeExtensions
{
    ...
    public static void RemoveChildren(this Node source, Func<Node, bool> where, bool recurse = true)
        => (recurse ? source.RecurseChildren() : source.GetChildren()).Where(where).ForEach(x => x.DetachChild(free: true));

    public static void RemoveChildren<T>(this Node source, Func<T, bool> where, bool recurse = true)
        => (recurse ? source.RecurseChildren<T>() : source.GetChildren<T>()).Where(where).ForEach(x => (x as Node).DetachChild(free: true));
