...
namespace F00F
{
    public static class RayCastExtensions
    {
        ...
        public static CollisionObject3D RayCastHitBody(this Node3D node, float distance) => node.RayCastHitBody(node.Position, node.Transform.Basis.Z, distance);
        public static CollisionObject3D RayCastHitBody(this Node3D node, in Vector3 direction, float distance) => node.RayCastHitBody(node.Position, direction, distance);
        public static CollisionObject3D RayCastHitBody(this Node3D node, in Vector3 rayStart, in Vector3 direction, float distance)
        {
            return RayCollision(node, rayStart, direction, distance, out var result)
                ? (CollisionObject3D)result["collider"].AsGodotObject()
                : null;
        }
