    ...
    public static class Node3DExtensions
    {
        ...
        public static Transform3D GlobalTransform(this Node3D node)
        {
            return node.TopLevel || !node.HasParent(out Node3D parent)
                ? node.Transform : node.Transform.TransformedBy(parent.GlobalTransform());
        }
        ...
        public static void FlipScale(this Node3D source, Vector3.Axis axis = Vector3.Axis.Z)
            => source.Scale = source.Scale.Flipped(axis);
    }
}
