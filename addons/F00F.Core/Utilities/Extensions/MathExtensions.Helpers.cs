...
public static class MathExtensions_Helpers
{
    ...
    public static bool IsZeroApprox(this float x) => Mathf.IsZeroApprox(x);

    public static bool Zero(this int x) => x.IsZero();
    public static bool Zero(this float x) => x.IsZero();

    public static float FF(this float to, float from, float? max = null)
    {
        const float Zero = 0;
        const float Flag = -1;

        return
            from is Flag ? to > from ? Zero : Flag :
            to < Zero ? Flag : max.HasValue ? to.ClampMax(max.Value) : to;
    }

    public static float? FF(this float value)
    {
        const float Flag = -1;
        return value is Flag ? null : value;
    }
}
