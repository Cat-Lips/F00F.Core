...
namespace F00F
{
    public static class MeshExtensions_Shapes
    {
        ...
        public static CollisionShape3D CreateCapsuleShape(this Mesh mesh, Axis axis = Axis.Y) => mesh.GetAabb().CreateCapsuleShape(axis);
        public static CollisionShape3D CreateCylinderShape(this Mesh mesh, Axis axis = Axis.Y) => mesh.GetAabb().CreateCylinderShape(axis);
        ...
        {
            var (height, radius, rotation) = GetRadialDimensions(bb.Size, bb.GetShortestAxisIndex());
        ...
        private static (float Height, float Radius, Vector3 Rotation) GetRadialDimensions(in Vector3 size, Axis axis)
        {
            return axis switch
            {
                Axis.X => (size.Z, Mathf.Max(size.X, size.Y) * .5f, new(Const.Deg90, 0, 0)),
                Axis.Y => (size.Y, Mathf.Max(size.X, size.Z) * .5f, Zero),
                Axis.Z => (size.X, Mathf.Max(size.Y, size.Z) * .5f, new(0, 0, Const.Deg90)),
