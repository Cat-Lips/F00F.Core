...
{
    public static string Str(this string source, string prefix = " ", string suffix = "")
        => source.IsNullOrEmpty() ? null : $"{prefix}{source}{suffix}";

    public static string StrIf<T>(this T source, bool @if, string prefix = "", string suffix = "")
        => @if ? $"{source}".Str(prefix, suffix) : null;
    ...
    public static bool Empty(this string source) => source.IsEmpty();
    public static bool NullOrEmpty(this string source) => source.IsNullOrEmpty();
    public static bool NullOrEmpty(this NodePath source) => source.IsNullOrEmpty();
    public static bool NullOrEmpty(this StringName source) => source.IsNullOrEmpty();

    public static string NullIfEmpty(this string source) => source.IsNullOrEmpty() ? null : source;
    public static NodePath NullIfEmpty(this NodePath source) => source.IsNullOrEmpty() ? null : source;
    public static StringName NullIfEmpty(this StringName source) => source.IsNullOrEmpty() ? null : source;
    ...
    public static string TryTrimPrefix(this string instance, string prefix) => prefix.IsNullOrEmpty() ? instance : instance.TrimPrefix(prefix);
    public static string TryTrimSuffix(this string instance, string suffix) => suffix.IsNullOrEmpty() ? instance : instance.TrimPrefix(suffix);
    ...
    public static bool AnyOfN(this string source, params IEnumerable<string> values) => source.IsAnyOfN(values);
    public static bool NotAnyOfN(this string source, params IEnumerable<string> values) => source.IsNotAnyOfN(values);

    public static string SplitFirst(this string source, char sep) => source.Split(sep, 2).First();
    public static string SplitFirst(this string source, string sep) => source.Split(sep, 2).First();
    ...
    public static string Capitalise(this string source)
    {
        ...
        IEnumerable<string> Words()
        {
            foreach (Match m in WordRegex().Matches(source))
            {
                ...
                yield return word.All(char.IsUpper) ? word : $"{char.ToUpperInvariant(word[0])}{word[1..]}";
            }
        }
    }

    [GeneratedRegex(@"[A-Z]{2,}(?=$|[A-Z][a-z])|[A-Z]?[a-z]+|[A-Z]|\d+", RegexOptions.Compiled)]
