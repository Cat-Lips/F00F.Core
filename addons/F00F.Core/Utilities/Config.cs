...
public class Config
{
    ...
    public T Get<[MustBeVariant] T>(string key, T dflt) => Get("", key, dflt);
    public T Get<[MustBeVariant] T>(string section, string key, T dflt)
        => TryGet<T>(section, key, out var value) ? value : dflt;
