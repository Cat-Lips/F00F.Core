...
public partial class PlayerList : DataView
{
    ...
    {
        myPlayers.Add(pid, (data, Rows.Add(Grid, pid, data, xtras)));
    ...
    private static class Rows
    {
        ...
        public static Control[] Add(GridContainer grid, int pid, PlayerData data, string[] xtras)
        {
            var controls = CreateRow(pid, data, xtras);
        ...
        private static Control[] CreateRow(int pid, PlayerData data, string[] xtras)
        {
            return Labels().InterspersedWith(() => UI.NewSep(v: true)).ToArray();
            ...
            IEnumerable<Control> Labels()
            {
                ...
                var lblScore = UI.NewLabel($"{key}_Score", $"{data.CurrentScore}", alignment: HorizontalAlignment.Right);

                data.RankChanged += x => lblRank.Text = $"{x}";
                data.NameChanged += x => lblName.Text = x;
                data.ScoreChanged += x => lblScore.Text = $"{x}";
