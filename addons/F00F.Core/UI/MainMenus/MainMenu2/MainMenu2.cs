...
public partial class MainMenu2 : RootPopup, IMainMenu
{
    ...
    private MainMenu MainMenu => field ??= GetNode<MainMenu>("%MainMenu");
    private PauseMenu PauseMenu => field ??= GetNode<PauseMenu>("%PauseMenu");
    private EndGameMenu EndGameMenu => field ??= GetNode<EndGameMenu>("%EndGameMenu");
    private StatusBar NetworkStatus => field ??= GetNode<StatusBar>("%NetworkStatus");
