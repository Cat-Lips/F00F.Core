...
public partial class Network : Node
{
    ...
    public void Initialise<TPlayer>(
        ...
        Action<TPlayer> PlayerAdded = null,
        Action<TPlayer> PlayerRemoved = null) where TPlayer : Node
    {
        SpawnTarget ??= Spawn;
        ...
        void RemovePlayer(int pid)
        {
            if (myPlayers.Remove(pid, out var player))
                player.GetParent().RemoveChild(player, free: true);
        ...
        {
            Log.Debug($"Player added: {player.Name} ({LocalStr(player.AuthId())})");
            PlayerAdded?.Invoke((TPlayer)player);
        }

        void OnPlayerRemoved(Node player)
        {
            Log.Debug($"Player removed: {player.Name} ({LocalStr(player.AuthId())})");
            PlayerRemoved?.Invoke((TPlayer)player);
        }

        Node CreatePlayer(int pid)
        {
            var player = Utils.New<TPlayer>();
